...
export class StockOutService {
    ...
    async findOne(store: Store, id: string) {
        const stockOut = await this.em.findOne(StockOut,
            { id, sale: { store } },
            { populate: STOCK_OUT_POPULATE },
        );
    ...
    async create(store: Store, dto: CreateStockOutDto): Promise<{ message: string }> {
        return await this.em.transactional(async (em) => {
            const [sale, inventory] = await Promise.all([
                em.findOne(Sale, { id: dto.saleId, store }, { populate: ['items', 'items.product'] }),
            ...
            const saleItemMap = new Map(sale.items.getItems().map(item => [item.id, item]));
            ...
            return { message: `Stock out created successfully with sequence ${this.sequenceService.formatSequence(sequence)}.` };
        });
    }

    async update(store: Store, id: string, dto: UpdateStockOutDto): Promise<{ message: string }> {
        return await this.em.transactional(async (em) => {
            const stockOut = await em.findOne(StockOut,
                { id, sale: { store } },
                { populate: ['items', 'items.product', 'items.saleItem', 'inventory', 'inventory.products', 'sale', 'sale.items'] }
            );
            ...
            if (dto.status) {
                this.validateStockOutTransition(stockOut.status as StockOutStatus, dto.status as StockOutStatus);
    ...
    private validateItems(items: StockOutItemDto[], saleItemMap: Map<string, SaleItem>): void {
        for (const item of items) {
            const saleItem = saleItemMap.get(item.saleItemId);
            if (!saleItem)
                throw new NotFoundException(`Sale item with id ${item.saleItemId} does not belong to this sale`);
        }
    }

    private validateStockOutTransition(currentStatus: StockOutStatus, newStatus: StockOutStatus): void {
